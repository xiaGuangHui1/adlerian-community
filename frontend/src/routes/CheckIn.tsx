...
const MOCK_PRACTICE_TOPIC = {
  theme: 'Accepting your imperfect self',
  description: 'Find one small thing today you felt you didn\'t do well enough, or that made you feel inferior, and try saying to yourself: "Even though I didn\'t do it well, I still accept and love who I am."',
  tags: ['Self-Acceptance', 'Separation of Tasks'],
};

const MOCK_CHECKINS = [
  { date: '2026-05-24', mood: '😊', theme: 'Separation of Tasks', content: 'Today a colleague was complaining about the boss again. I used to get swept up in her emotions and feel down with her. Today I tried separation of tasks: her emotions are her task, and I don\'t need to take responsibility for them. I listened politely for a while, then went back to my own work feeling very calm.' },
  { date: '2026-05-23', mood: '💪', theme: 'Courage to face difficulty', content: 'I finally shared my suggestion in a meeting. My heart was racing and my hands were shaking, but I told myself it was okay to be challenged — what mattered was that I expressed myself. To my surprise, everyone found my suggestion valuable! It felt amazing.' },
  { date: '2026-05-22', mood: '🍃', theme: 'Living in the moment', content: 'On the way home from work I didn\'t play on my phone — instead I noticed the flowers and the sunset by the roadside. I realized life is made of these tiny moments. The past doesn\'t matter, the future hasn\'t come, only the present is real.' },
];
...
export default function CheckInPage() {
  ...
  const todayStr = todayDate.toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric', weekday: 'long' });
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              ...
              <div className="relative z-10">
                <div className="flex items-center justify-between mb-8">
                  <div>
                    ...
                    <h1 className="text-3xl font-bold">Do you have the courage to change today?</h1>
                  </div>
                  <div className="text-right">
                    <span className="text-4xl font-black text-peach-500">Day {streak || 1}</span>
                    <p className="text-xs text-gray-400 mt-1 uppercase tracking-widest">day streak</p>
                  </div>
                </div>
                ...
                <div className="bg-warm-50 rounded-2xl p-6 mb-8 border border-orange-100/50">
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <h3 className="font-bold text-lg">Today's Practice Theme</h3>
                  </div>
            ...
            <div className="bg-white rounded-3xl p-6 shadow-sm border border-orange-50">
              ...
              <div className="pt-6 border-t border-orange-50 flex items-center justify-around">
                ...
                <div className="text-center">
                  ...
                  <p className="text-[10px] text-gray-400 uppercase">Total courage</p>
                </div>
              </div>
            </div>
          </div>
          ...
          {selectedDay && (
            <div className="bg-white p-6 rounded-3xl shadow-sm border border-orange-50 mb-8">
              <div className="flex items-center justify-between mb-3">
                ...
                <button onClick={() => setSelectedDay(null)} className="text-xs text-gray-400 cursor-pointer border-0 bg-transparent hover:text-gray-600">Close</button>
              </div>
          ...
          <div className="flex flex-col lg:flex-row gap-8">
            ...
            <div className="lg:w-2/3 space-y-6">
                      ...
                      <div className="flex justify-between items-start mb-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-gray-400">Theme: {mc.theme}</p>
                          </div>
                        </div>
                        <button className="text-gray-300 hover:text-peach-500 bg-transparent border-0 cursor-pointer">
                          <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 256 256"><path d="M227.31,73.37,182.63,28.68a16,16,0,0,0-22.63,0L36.69,152A15.86,15.86,0,0,0,32,163.31V208a16,16,0,0,0,16,16H92.69A15.86,15.86,0,0,0,104,219.31L227.31,96a16,16,0,0,0,0-22.63ZM51.31,160,136,75.31,152.69,92,68,176.68ZM48,179.31,76.69,208H48Zm48,25.38L79.31,188,164,103.31,180.69,120Zm96-96L147.31,64l24-24L216,84.68Z"/></svg>
              ...
              >
                {showHistory ? 'Collapse' : 'View more history'}
              </button>
            </div>
            ...
            <aside className="lg:w-1/3 space-y-8">
              ...
              <div className="bg-white p-6 rounded-3xl shadow-sm border border-orange-50">
                ...
                <div className="space-y-6">
                    ...
                    <div key={item.id} className="flex gap-3">
                      ...
                      <div className="flex-grow">
                        <p className="text-xs">
                          ...
                          <span className="text-gray-400 font-normal"> completed a check-in</span>
                        </p>
                  ...
                  {feed.length === 0 && (
                    <p className="text-xs text-gray-400 text-center py-4">No community activity yet — complete today's check-in</p>
                  )}
                </div>
              </div>
                  ...
                  {myChallenges.length > 0 ? (
                    myChallenges.slice(0, 2).map((c) => (
                      <div key={c.id} className={`p-4 rounded-2xl border group cursor-pointer hover:shadow-md transition-all ${
                        ...
                        <p className="text-[10px] text-gray-400 mb-3">1,240 participants</p>
                        {c.enrolled ? (
                          <>
                            ...
                            <div className="flex justify-between items-center mt-2">
                              <span className="text-[10px] font-bold text-peach-500">{c.progress}/{c.targetCount} days</span>
                        ...
                        {c.completed && (
                          <span className="text-[10px] text-teal-700 bg-teal-50 px-2 py-0.5 rounded-full">Completed</span>
                        )}
                      </div>
                    ))
                  ) : (
                    <>
                      <div className="p-4 rounded-2xl bg-gradient-to-br from-orange-50 to-white border border-orange-100 group cursor-pointer hover:shadow-md transition-all">
                        <h4 className="font-bold text-sm mb-1 group-hover:text-peach-500">21-Day Self-Acceptance Challenge</h4>
                        <p className="text-[10px] text-gray-400 mb-3">1,240 participants</p>
                        <div className="w-full bg-gray-100 h-1.5 rounded-full overflow-hidden">
                          <div className="bg-peach-500 h-full rounded-full" style={{ width: '70%' }} />
                        </div>
                        <div className="flex justify-between items-center mt-2">
                          <span className="text-[10px] font-bold text-peach-500">15/21 days</span>
                          <svg className="w-4 h-4 text-gray-300 group-hover:text-peach-500" fill="currentColor" viewBox="0 0 256 256"><path d="M221.66,133.66l-72,72a8,8,0,0,1-11.32-11.32L196.69,136H40a8,8,0,0,1,0-16H196.69L138.34,61.66a8,8,0,0,1,11.32-11.32l72,72A8,8,0,0,1,221.66,133.66Z"/></svg>
                        </div>
                      </div>
                      <div className="p-4 rounded-2xl bg-gradient-to-br from-teal-50 to-white border border-teal-100 group cursor-pointer hover:shadow-md transition-all">
                        <h4 className="font-bold text-sm mb-1 group-hover:text-teal-500">7-Day Separation of Tasks</h4>
                        <p className="text-[10px] text-gray-400 mb-3">3,560 participants</p>
                        <button className="w-full py-2 bg-teal-500 text-white text-[10px] font-bold rounded-xl hover:bg-opacity-90 cursor-pointer border-0">
                          Join Now
                        </button>
                      </div>
              ...
              <div className="bg-brown-900 p-8 rounded-3xl text-warm-50 relative overflow-hidden group">
                ...
                <h4 className="font-bold text-lg mb-4 relative z-10">Courage Moment</h4>
                <p className="text-sm italic leading-relaxed text-gray-300 mb-6 relative z-10">
                  {quote ? `"${quote.content}"` : '"The so-called freedom is being disliked by others."'}
                </p>
                <div className="text-right relative z-10">
                  <p className="text-xs font-bold text-peach-500">
                    —— {quote?.source || 'The Courage to Be Disliked'}
                  </p>
                </div>
              </div>
            </aside>
